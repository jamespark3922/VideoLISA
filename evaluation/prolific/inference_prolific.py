...
def parse_args(args):
    ...
    parser.add_argument("--meta_exp_path", default="", type=str)
    parser.add_argument("--video_folder", default="", type=str)
    parser.add_argument("--vis_save_path", default="./results/VideoLISA-Prolific/", type=str)
...
def main(args):
    ...
    # ---------------------------- read data ------------------------------------
    meta_exp_path = args.meta_exp_path
    meta_exp = load_from_disk(meta_exp_path)
    ...
    job_list = meta_exp['id']

    job_list_subset = [(job_list[i], meta_exp[i]['exp']) for i in range(len(job_list)) if i % args.subset_num == args.subset_idx]

    total_infer = len(job_list_subset)
    progress_bar = tqdm(total=total_infer, desc='Progress {}'.format(args.subset_idx))

    # ------------------ start processing -------------------------
    for id, ref_query in job_list_subset:
        vid_id, exp_id = id.rsplit('_', 1)
        # prepare folder
        ...
        save_dir_vid_exp = os.path.join(args.vis_save_path, id)
        ...
        print(f"Finished inference for {id}, results saved in {save_dir_vid_exp}")
